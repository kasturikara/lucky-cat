...
function SubsKosPages() {
  const [loading, setLoading] = useState(true);
  const [subsKos, setSubsKos] = useState([]);

  useEffect(() => {
    setLoading(true);
    const fetchData = async () => {
      const subsKos = await getSubsByJenis("kos");
      setSubsKos(subsKos);
      setLoading(false);
    };
    fetchData();
  ...
  return (
    <div>
      <div className="p-4 mt-2 rounded-lg bg-slate-50">
        <h1 className="text-3xl font-bold">LIST KRITERIA PENILAIAN KOSTUM</h1>
      </div>
