...
function SubsPbbPages() {
  ...
  const [subsPbb, setSubsPbb] = useState([]);

  useEffect(() => {
    setLoading(true);
    const fetchData = async () => {
      const subsPbb = await getSubsByJenis("pbb");
      setSubsPbb(subsPbb);
      setLoading(false);
    };
    fetchData();
  ...
  return (
    <div>
      <div className="p-4 mt-2 rounded-lg bg-slate-50">
        <h1 className="text-3xl font-bold">LIST KRITERIA PENILAIAN PBB</h1>
      </div>
