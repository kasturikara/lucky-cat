...
function SubsDanPages() {
  ...
  const [subsDan, setSubsDan] = useState([]);

  useEffect(() => {
    setLoading(true);
    const fetchData = async () => {
      const subsDan = await getSubsByJenis("dan");
      setSubsDan(subsDan);
      setLoading(false);
    };
    fetchData();
  ...
  return (
    <div>
      <div className="p-4 mt-2 rounded-lg bg-slate-50">
        <h1 className="text-3xl font-bold">LIST KRITERIA PENILAIAN DANTON</h1>
      </div>
