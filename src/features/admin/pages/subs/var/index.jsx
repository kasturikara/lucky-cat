...
function SubsVarPages() {
  ...
  const [subsVar, setSubsVar] = useState([]);

  useEffect(() => {
    setLoading(true);
    const fetchData = async () => {
      const subsVar = await getSubsByJenis("var");
      setSubsVar(subsVar);
      setLoading(false);
    };
    fetchData();
  ...
  return (
    <div>
      <div className="p-4 mt-2 rounded-lg bg-slate-50">
        <h1 className="text-3xl font-bold">LIST KRITERIA PENILAIAN VARIASI</h1>
      </div>
